...
## 尤度の性質
色々と性質を挙げられるとは思うが、ここでは和が 1 になるとは限らないとう性質と最尤推定についてのみ述べる。

### 和が 1 になるとは限らない
これは標語的に言えば「確率値は和が 1 になるように規格化されているが、パラメタの方で和を取ってもその限りではない」ということになる。
以上、ということで終わりにしてもいいが、1 になることもあるので混乱する人がいるかもしれないので、例を挙げて見てみる。
...
和が 1 にならない例として Bernoulli 分布を考えてみる。
...
たまに確率密度関数の値が 1 を超えるという話と混同しているものがあるが、和が 1 にならないというのは離散的な確率を取り扱っていても起こりうるものである。
...
尤度は定義により 0 以上であるので log を取る（計算機上で扱いやすい）。
...
ちょっとした違いで結果が変わりうることが分かるので、論文とかで報告されている実験を再現することはそりゃ難しいよねと優しい気持ちになる（ならない。ちゃんと再現できるように手順を構築するのは義務でしょ）。
...
ただし尤度関数として関数として捉える場合には、これまで説明したように $$d$$ を固定して $$h$$ の関数と見る。
...
実験計画に依存するか否かというの、誰が最初に考えたかは知らないが、なかなか興味深いよなあ。
