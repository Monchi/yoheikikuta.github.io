...
せっかくなので本の一冊でも読みながら勉強しようと思って選んだのが「すごいHASKELLたのしく学ぼう！」である。
これは原著で「Learn You a Haskell For Great Good!」というものがあり自分は大体原著の方を読んでいくタイプだが、翻訳本訳者の一人である村主氏に追悼の意を示すという意味で翻訳本の方を読んでみることにした。
...
型シグネチャの残りの部分は `a -> a -> a` という形で、これは 2 つの引数を取って 1 つの結果（大きい方）を返すという風に読む。
...
ちなみに `map` はこう定義されていて、この段階の知識でも `(a -> b)` という関数とリスト `[a]` を取り、関数を適用させたリスト `[b]` を返すと読むことができる。
...
この `[]` は 1 つの具体型 `a` を取って `[a]` を返すものになっているので、`kind` がお望みのものになっている。
ちなみに、Haskell における `[a]` は `a : []` の syntax sugar である。

これで `fmap` の定義を書き直すと `fmap :: (a -> b) -> [a] -> [b]` となる。
これは... `map` だ。
`map` とは `fmap` を `[]` の場合に限定した特別な場合だったということが分かる。
...
### Applicative Functor
次いで `Applicative` である（以降では後ろの `Functor` を省略して `Applicative` のみで書いていく）。
...
しんどくなってきたら具体例を見よう。
`Functor` の時と同じく `[]` を扱う。
`[]` は `Funcotr` だったが `Applicative` でもある。
まずは `(<*>)` の方から。
...
それでは `Applicative` が満たすべき性質 `Applicative` 則を列挙すると以下のものである。
...
これは結構難しい。
少なくとも今の自分には `Applicative` 足りうるためになぜこれらが必要なのかということを説明できない。
ここでは `[]` を例にとりそれぞれを見ていくことにする。
...
次は `(>>)` で、これは典型的には `x >> y = x >>= _ -> y` という実装になり、1 つ目の引数の `Monad` 値に対してそれを無視した上で 2 つ目の `Monad` 値を返すものになっている。
...
というわけで、`do` 記法は `Monad` の `>>=` に関する syntax sugar に他ならないというものでした。
...
それもそのはずで、リスト内包表記も `[] Monad` の syntax sugar だったのさ、というオチです。
ただ、ここまで読めた人なら、単に「リスト内包表記は `Monad` の syntax sugar ですよ」という statement が血肉の通わない空虚なものとしてではなく、なぜ `Monad` を使うことでこのようなことが実現できているのかを明快に説明できるようになっているはず。
